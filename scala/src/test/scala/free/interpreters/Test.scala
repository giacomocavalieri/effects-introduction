...
object Test:
  ...
  def testCoinFlipInterpreter[M[_]: Monad](coinValue: Boolean) = new (CoinFlipDSL ~> M):
    override def apply[A](c: CoinFlipDSL[A]): M[A] = c match
      case FlipCoin => coinValue.pure

  def testConsoleInterpreter = new (ConsoleDSL ~> Writer[String, _]):
    override def apply[A](c: ConsoleDSL[A]): Writer[String, A] = c match
      case PrintLine(msg) => Writer.tell(msg)

  def testAppInterpreter(coinValue: Boolean) = new (AppDSL ~> Writer[String, _]):
